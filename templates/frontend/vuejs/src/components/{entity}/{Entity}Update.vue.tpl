    ...
    foreach (var field in Model.GetOneRelations())
    {
              <div class="form-group">
                  ...
                  <select formControlName="@field.Name" class="form-control" v-model="@(Model.Name).@field.Name">
                      <option v-for="@field.Name in @field.Name.Pluralize()" :value="@(Model.Name).@field.Name" :key="@(field.Name).id">
...
<script>
export default {
    name: '@Model.Name.FirstLetterToUpper()Update',
    data: () => {
        return { 
            @Model.Name: {},
@if (Model.HasOneRelations())
...
<text>        this.@field.Name.Pluralize() = (await this.$api.get('@field.Name.Pluralize()')).data
    ...
    methods: {
        async save () {
            await this.$api.update('@Model.Name.Pluralize()', this.$route.params.id, this.@Model.Name)
